/* ============================================================
   ...
   Các biến cần có trong Cloudflare (xem file cai-dat-cloudflare.html):
     ...
     ADMIN_EMAIL     — (tuỳ chọn) email nhận MÃ XÁC NHẬN đăng nhập quản trị;
                       để trống thì dùng địa chỉ đầu tiên trong LEAD_EMAIL
     SUPPORT_FROM    — (tuỳ chọn) người gửi mã, mặc định "ROOT & RISE LAB <support@sptlab.co>"
...
export async function taoBang(db) {
    ...
    await db.batch([
        ...
        db.prepare(`CREATE TABLE IF NOT EXISTS chan_dang_nhap (
            ...
            ma TEXT, ma_het INTEGER DEFAULT 0, ma_sai INTEGER DEFAULT 0, ma_gui INTEGER DEFAULT 0, ma_so_lan INTEGER DEFAULT 0)`),
        /* đếm thư đã gửi qua Resend, để báo trước lúc gần chạm giới hạn gói miễn phí
           (100 thư/ngày · 3.000 thư/tháng — Resend tính theo giờ UTC) */
        db.prepare(`CREATE TABLE IF NOT EXISTS thu_da_gui (
            id INTEGER PRIMARY KEY AUTOINCREMENT, ts INTEGER NOT NULL,
            ngay TEXT NOT NULL, thang TEXT NOT NULL, so_nguoi INTEGER DEFAULT 1, loai TEXT)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS thu_ngay ON thu_da_gui(ngay)`),
        db.prepare(`CREATE INDEX IF NOT EXISTS thu_thang ON thu_da_gui(thang)`),
        /* chặn đếm trùng: 1 máy (IP) quét/mở lại nhiều lần trong 30 phút chỉ tính 1 lượt.
           Chỉ lưu DẤU VÂN TAY đã băm của IP (một chiều, không suy ngược ra IP thật),
           không lưu địa chỉ IP — đúng tinh thần như bảng chan_dang_nhap ở trên. */
        db.prepare(`CREATE TABLE IF NOT EXISTS chan_trung (
            k TEXT PRIMARY KEY, lan_cuoi INTEGER NOT NULL)`),
        /* chặn spam gửi form liên hệ: 1 máy (IP) gửi quá nhiều lần trong ít phút thì
           tạm khoá vài phút. Cùng cách băm một chiều như các bảng "chan_*" ở trên,
           không lưu địa chỉ IP thật. */
        db.prepare(`CREATE TABLE IF NOT EXISTS chan_gui (
            k TEXT PRIMARY KEY, dem INTEGER DEFAULT 0, bat_dau INTEGER NOT NULL)`)
    ]);
    /* Bảng đã tạo từ trước thì CREATE TABLE IF NOT EXISTS không thêm cột mới.
       Thêm tay từng cột, cột nào có rồi thì lệnh báo lỗi — bỏ qua là xong. */
    for (const [bang, cot] of [
        ['chan_dang_nhap', 'ma TEXT'],
        ['chan_dang_nhap', 'ma_het INTEGER DEFAULT 0'],
        ['chan_dang_nhap', 'ma_sai INTEGER DEFAULT 0'],
        ['chan_dang_nhap', 'ma_gui INTEGER DEFAULT 0'],
        ['chan_dang_nhap', 'ma_so_lan INTEGER DEFAULT 0']
    ]) {
        try { await db.prepare(`ALTER TABLE ${bang} ADD COLUMN ${cot}`).run(); } catch (e) { }
    }
    daTaoBang = true;
}
...
/* ---------- gửi thư qua Resend ---------- */
export async function guiThu(env, { subject, html, text, den: denRieng, tu, loai }) {
    const den = (denRieng && denRieng.length ? denRieng
        : String(env.LEAD_EMAIL || '').split(',')).map(s => String(s).trim()).filter(Boolean);
    ...
    try {
        const r = await fetch(env.RESEND_URL || 'https://api.resend.com/emails', {
            ...
            body: JSON.stringify({ from: tu || env.MAIL_FROM || 'ROOT & RISE LAB <thongbao@sptlab.co>', to: den, subject, html, text })
        });
        if (!r.ok) return { ok: false, err: 'Resend ' + r.status + ': ' + gon(await r.text(), 200) };
        /* ghi lại đã gửi mấy thư — dùng để báo trước lúc gần chạm giới hạn gói
           miễn phí của Resend. Lỗi ghi thì bỏ qua, không làm hỏng việc gửi thư. */
        if (env.DB) {
            try {
                const iso = new Date().toISOString();
                await env.DB.prepare(
                    `INSERT INTO thu_da_gui (ts, ngay, thang, so_nguoi, loai) VALUES (?, ?, ?, ?, ?)`
                ).bind(Date.now(), iso.slice(0, 10), iso.slice(0, 7), den.length, loai || '').run();
            } catch (e) { }
        }
        return { ok: true };
    } catch (e) {
