/* Nội dung trang ROOTLAB — sửa bằng trang quản trị rồi tải file này về, ghi đè lên đây.
   lines   = các DÒNG SẢN PHẨM ở màn đầu. Mỗi dòng là một trang riêng.
   products= các chai. Một dòng có thể gồm nhiều chai (trường "sp" của dòng).
   i18n    = bản dịch. Khối nào không khai thì tự dùng bản tiếng Việt. */
window.ROOTLAB_CONTENT = {
  "brand": "ROOTLAB",
  "contactBtn": "Liên hệ",
  "nav": [
    "Trang chủ",
    "Thành phần",
    "Cách dùng",
    "Hiệu quả",
    "Đánh giá",
    "Giá"
  ],
  "hero": {
    "title1": "Awaken",
    "title2": "Roots",
    "cta": "Mua bộ đôi",
    "badgeTitle": "CAM KẾT",
    "badgeSub": "KHÔNG SULFATE · KHÔNG PARABEN",
    "side1": "Naturally",
    "side2": "Strong",
    "scrollHint": "CUỘN XUỐNG"
  },
  "lines": [
    {
      "key": "phuchoi",
      "name": "Bộ phục hồi chân tóc",
      "navLabel": "Phục hồi & Mọc tóc",
      "navKicker": "Dòng 01",
      "nguoi": "growth.jpg",
      "accent": "#12a08b",
      "sp": [
        "cleanse",
        "serum"
      ],
      "heroT1": "Awaken",
      "heroT2": "Roots",
      "cta": "Mua bộ đôi"
    },
    {
      "key": "goi",
      "name": "Dầu gội dưỡng chân tóc",
      "navLabel": "Dầu gội",
      "navKicker": "Dòng 02",
      "nguoi": "shampoo.jpg",
      "accent": "#12a2bd",
      "sp": [
        "goi"
      ],
      "heroT1": "Daily",
      "heroT2": "Clean",
      "cta": "Mua dầu gội",
      "pricing": {
        "eyebrow": "BẢNG GIÁ",
        "t1": "Start",
        "t2": "Today",
        "comboLabel": "",
        "comboOld": "",
        "comboNew": "",
        "comboBtn": "",
        "comboLink": "",
        "note": "Giá minh hoạ — thay bằng giá bán thật của bạn."
      },
      "ingredients": {
        "eyebrow": "THÀNH PHẦN",
        "t1": "Gentle",
        "t2": "Wash",
        "lead": "Bọt từ dừa thay cho sulfate: đủ sạch cho tóc bết, nhưng không bóc mất lớp dầu đang bảo vệ da đầu.",
        "items": [
          {
            "chip": "01",
            "name": "Coco Glucoside",
            "desc": "Chất tạo bọt gốc dừa. Rửa trôi bã nhờn mà không làm da đầu khô căng sau khi gội"
          },
          {
            "chip": "02",
            "name": "Panthenol B5 & Glycerin",
            "desc": "Giữ nước cho da đầu và sợi tóc, tóc mềm sau khi sấy, bớt xơ ở phần ngọn"
          },
          {
            "chip": "03",
            "name": "Bạc hà & tràm trà",
            "desc": "Làm dịu da đầu ngứa, mát nhẹ lúc gội, hỗ trợ kiểm soát gàu do dầu"
          }
        ]
      },
      "howto": {
        "eyebrow": "CÁCH DÙNG",
        "t1": "Daily",
        "t2": "Ritual",
        "lead": "Gội đúng cách quan trọng ngang với gội bằng gì. Ba việc, mỗi lần chưa tới ba phút.",
        "steps": [
          {
            "no": "01",
            "h": "Làm ướt tóc thật kỹ",
            "p": "Xả nước ấm khoảng 30 giây cho tóc ngấm đều. Tóc ướt hẳn thì chỉ cần rất ít dầu gội đã đủ bọt.",
            "em": "NƯỚC ẤM, KHÔNG NÓNG"
          },
          {
            "no": "02",
            "h": "Massage da đầu 60 giây",
            "p": "Lấy lượng bằng đồng xu, xoa ra hai lòng bàn tay rồi massage bằng đầu ngón tay — gội da đầu, không chà thân tóc.",
            "em": "MỖI NGÀY HOẶC CÁCH NGÀY"
          },
          {
            "no": "03",
            "h": "Xả sạch, thấm khô",
            "p": "Xả tới khi tóc hết trơn. Dùng khăn thấm thay vì vò, tóc ướt rất dễ gãy.",
            "em": "THẤM, ĐỪNG VÒ"
          }
        ]
      },
      "results": {
        "eyebrow": "HIỆU QUẢ",
        "t1": "Clean",
        "t2": "Scalp",
        "lead": "Dầu gội không làm tóc mọc. Việc của nó là giữ da đầu ở trạng thái để tóc mọc được.",
        "weeks": [
          {
            "b": "Ngay lần đầu",
            "s": "Tóc nhẹ, không còn cảm giác bết"
          },
          {
            "b": "Tuần 2",
            "s": "Da đầu bớt ngứa, giảm gàu do dầu"
          },
          {
            "b": "Tuần 4",
            "s": "Giãn được thời gian giữa hai lần gội"
          },
          {
            "b": "Tuần 8",
            "s": "Da đầu ổn định, tóc vào nếp dễ hơn"
          }
        ],
        "tiles": [
          {
            "val": "0%",
            "lbl": "Sulfate và silicone trong công thức"
          },
          {
            "val": "5,2",
            "lbl": "Độ pH, gần với độ pH tự nhiên của da đầu"
          },
          {
            "val": "300ml",
            "lbl": "Đủ dùng khoảng 10 tuần nếu gội cách ngày"
          }
        ],
        "note": "Số liệu minh hoạ — thay bằng kết quả kiểm nghiệm thật của bạn trước khi phát hành trang."
      },
      "reviews": {
        "eyebrow": "ĐÁNH GIÁ",
        "t1": "Real",
        "t2": "Voices",
        "items": [
          {
            "stars": 5,
            "text": "Tóc mình bết tới mức phải gội mỗi ngày. Đổi sang dầu gội này thì cách ngày gội vẫn thấy ổn, da đầu không còn ngứa.",
            "name": "Khánh Vy, 26 tuổi",
            "meta": "Đã dùng 2 tháng"
          },
          {
            "stars": 5,
            "text": "Tóc mình nhuộm nên sợ dầu gội làm phai màu. Dùng chai này gần ba tháng, màu xuống chậm hơn hẳn loại cũ.",
            "name": "Ngọc Trâm, 31 tuổi",
            "meta": "Đã dùng 3 tháng"
          },
          {
            "stars": 4,
            "text": "Bọt ít hơn dầu gội thường nên lúc đầu hơi lạ, quen rồi thì thấy tóc mềm và nhẹ hơn. Trừ nửa sao vì chai hơi khó bóp lúc gần hết.",
            "name": "Trọng Nghĩa, 34 tuổi",
            "meta": "Đã dùng 6 tuần"
          }
        ]
      }
    }
  ],
  "products": [
    {
      "key": "cleanse",
      "name": "Deep Cleanse",
      "cardSub": "Bước 1 · Làm sạch",
      "tag": "Bước 1 · Làm sạch chân tóc",
      "desc": "Gel làm sạch sâu chân tóc và nang tóc.<br>Cuốn trôi bã nhờn, bụi bẩn và tế bào chết<br>đang bít lỗ chân lông trên da đầu.",
      "photo": "",
      "bottle": {
        "body": "#f1f7f4",
        "cap": "#0d2f27",
        "ink": "#0b3d33",
        "l1": "DEEP",
        "l2": "CLEANSE",
        "sub": "SCALP & FOLLICLE",
        "vn": "Làm sạch chân tóc & nang tóc",
        "vol": "200 ml"
      },
      "theme": {
        "inner": "#0b8a78",
        "mid": "#044e3b",
        "outer": "#011411"
      },
      "price": "390.000",
      "priceSub": "Gel làm sạch chân tóc & nang tóc · 200ml",
      "features": [
        "Dùng được khoảng 8 tuần",
        "Không sulfate, không paraben",
        "Hợp da đầu dầu và có gàu"
      ],
      "buyBtn": "Mua Deep Cleanse",
      "buyLink": "",
      "shape": "jar"
    },
    {
      "key": "serum",
      "name": "Growth Serum",
      "cardSub": "Bước 2 · Dưỡng tóc",
      "tag": "Bước 2 · Dưỡng & kích thích mọc tóc",
      "desc": "Serum nuôi dưỡng nang tóc từ bên trong.<br>Thẩm thấu vào chân tóc, kích thích mọc tóc mới<br>và giảm gãy rụng theo từng ngày.",
      "photo": "",
      "bottle": {
        "body": "#a3600d",
        "cap": "#141009",
        "ink": "#fff6e2",
        "l1": "GROWTH",
        "l2": "SERUM",
        "sub": "ROOT ACTIVATOR",
        "vn": "Dưỡng tóc & kích thích mọc tóc",
        "vol": "60 ml"
      },
      "theme": {
        "inner": "#8a5f12",
        "mid": "#3a2606",
        "outer": "#120a01"
      },
      "price": "690.000",
      "priceSub": "Serum dưỡng & kích thích mọc tóc · 60ml",
      "features": [
        "Dùng được khoảng 6 tuần",
        "Không nhờn, không cần xả lại",
        "Dùng được cho cả nam và nữ"
      ],
      "buyBtn": "Mua Growth Serum",
      "buyLink": "",
      "shape": "dropper"
    },
    {
      "key": "goi",
      "name": "Root Shampoo",
      "cardSub": "Gội hằng ngày",
      "tag": "Dầu gội dịu nhẹ · không sulfate",
      "desc": "Dầu gội dùng được mỗi ngày.<br>Sạch vừa đủ, giữ lại lớp dầu tự nhiên<br>đang bảo vệ da đầu và chân tóc.",
      "photo": "",
      "bottle": {
        "body": "#eaf4f3",
        "cap": "#0a3b45",
        "ink": "#08414c",
        "l1": "ROOT",
        "l2": "SHAMPOO",
        "sub": "DAILY GENTLE WASH",
        "vn": "Dầu gội dịu nhẹ mỗi ngày",
        "vol": "300 ml"
      },
      "theme": {
        "inner": "#0f93a8",
        "mid": "#07485a",
        "outer": "#02161c"
      },
      "price": "290.000",
      "priceSub": "Dầu gội dịu nhẹ không sulfate · 300ml",
      "features": [
        "Dùng được khoảng 10 tuần",
        "Không sulfate, không silicone",
        "Gội được mỗi ngày, hợp cả tóc nhuộm"
      ],
      "buyBtn": "Mua dầu gội",
      "buyLink": "",
      "shape": "jar"
    }
  ],
  "ingredients": {
    "eyebrow": "THÀNH PHẦN",
    "t1": "What's",
    "t2": "Inside",
    "lead": "Hai công thức bổ trợ nhau: một bên dọn sạch đường đi cho nang tóc, một bên đưa dưỡng chất vào tận chân tóc.",
    "items": [
      {
        "chip": "B1",
        "name": "Salicylic Acid 2%",
        "desc": "Làm tan bã nhờn và lớp sừng đang bít miệng nang tóc"
      },
      {
        "chip": "B1",
        "name": "Tinh dầu tràm trà",
        "desc": "Kháng khuẩn nhẹ, giảm ngứa và gàu trên da đầu"
      },
      {
        "chip": "B1",
        "name": "Panthenol B5",
        "desc": "Làm dịu và giữ ẩm cho da đầu sau khi làm sạch sâu"
      },
      {
        "chip": "B2",
        "name": "Redensyl 3%",
        "desc": "Đánh thức nang tóc đang ngủ, kéo dài giai đoạn mọc"
      },
      {
        "chip": "B2",
        "name": "Caffeine & Biotin B7",
        "desc": "Tăng tuần hoàn dưới da đầu và cung cấp nguyên liệu tạo sợi tóc"
      },
      {
        "chip": "B2",
        "name": "Tinh dầu hương thảo",
        "desc": "Hỗ trợ giảm rụng tóc theo mùa"
      }
    ],
    "note": "Danh sách mẫu — thay bằng bảng thành phần thật trên nhãn sản phẩm của bạn."
  },
  "howto": {
    "eyebrow": "CÁCH DÙNG",
    "t1": "Two",
    "t2": "Steps",
    "lead": "Không phức tạp. Làm sạch vài lần một tuần, dưỡng mỗi tối trước khi ngủ.",
    "steps": [
      {
        "no": "01",
        "h": "Làm sạch chân tóc",
        "p": "Làm ướt tóc, lấy một lượng gel bằng đồng xu, massage trực tiếp lên da đầu 60 giây rồi xả thật kỹ.",
        "em": "2–3 LẦN / TUẦN"
      },
      {
        "no": "02",
        "h": "Nhỏ serum lên da đầu",
        "p": "Tóc còn hơi ẩm, rẽ ngôi và chấm serum dọc theo đường rẽ, đặc biệt ở vùng tóc thưa.",
        "em": "MỖI TỐI"
      },
      {
        "no": "03",
        "h": "Massage 1 phút",
        "p": "Xoa nhẹ bằng đầu ngón tay theo vòng tròn để serum thấm xuống nang tóc. Không cần xả lại.",
        "em": "KHÔNG RỬA LẠI"
      }
    ]
  },
  "results": {
    "eyebrow": "HIỆU QUẢ",
    "t1": "Week",
    "t2": "By Week",
    "lead": "Tóc mọc theo chu kỳ, không theo ngày. Đây là mốc thời gian thực tế bạn nên kỳ vọng.",
    "weeks": [
      {
        "b": "Tuần 2",
        "s": "Da đầu sạch, hết bết và ngứa"
      },
      {
        "b": "Tuần 4",
        "s": "Tóc rụng khi gội giảm rõ"
      },
      {
        "b": "Tuần 8",
        "s": "Tóc con nhú lên ở vùng thưa"
      },
      {
        "b": "Tuần 12",
        "s": "Tóc dày hơn thấy được bằng mắt"
      }
    ],
    "tiles": [
      {
        "val": "89%",
        "lbl": "Người dùng thấy giảm rụng sau 8 tuần"
      },
      {
        "val": "3,2×",
        "lbl": "Số sợi tóc con mới so với trước khi dùng"
      },
      {
        "val": "12",
        "lbl": "Số tuần trung bình để thấy tóc dày lên"
      }
    ],
    "note": "Số liệu minh hoạ — thay bằng kết quả khảo sát thật của bạn trước khi phát hành trang."
  },
  "reviews": {
    "eyebrow": "ĐÁNH GIÁ",
    "t1": "Real",
    "t2": "Voices",
    "items": [
      {
        "stars": 5,
        "text": "Da đầu mình dầu kinh khủng, gội hôm trước hôm sau đã bết. Dùng bước 1 được ba tuần thì tóc nhẹ hẳn, không còn ngứa nữa.",
        "name": "Minh Anh, 29 tuổi",
        "meta": "Đã dùng 3 tháng"
      },
      {
        "stars": 5,
        "text": "Mình rụng tóc sau sinh, hai bên thái dương thưa hẳn. Sau hai tháng dùng serum thì thấy lớp tóc con mọc lên, hơi lún phún nhưng có thật.",
        "name": "Thu Hà, 33 tuổi",
        "meta": "Đã dùng 2 tháng"
      },
      {
        "stars": 4,
        "text": "Serum không nhờn, để qua đêm sáng dậy tóc vẫn khô ráo. Trừ nửa sao vì mùi hương thảo hơi nồng với mình.",
        "name": "Đức Huy, 27 tuổi",
        "meta": "Đã dùng 6 tuần"
      },
      {
        "stars": 5,
        "text": "Dùng đủ bộ đôi mới thấy khác. Trước mình chỉ xịt serum mà da đầu còn bẩn nên gần như không ăn thua.",
        "name": "Lan Phương, 41 tuổi",
        "meta": "Đã dùng 4 tháng"
      }
    ],
    "note": "Đánh giá mẫu — thay bằng phản hồi thật của khách hàng."
  },
  "pricing": {
    "eyebrow": "BẢNG GIÁ",
    "t1": "Start",
    "t2": "Today",
    "comboLabel": "Mua cả bộ đôi",
    "comboOld": "1.080.000đ",
    "comboNew": "980.000đ",
    "comboBtn": "Mua bộ đôi",
    "comboLink": "",
    "note": "Giá minh hoạ — thay bằng giá bán thật của bạn."
  },
  "footer": {
    "tagline": "Chăm sóc tóc từ chân tóc — làm sạch trước, nuôi dưỡng sau.",
    "copyright": "© 2026 ROOTLAB"
  },
  "i18n": {
    "en": {
      "contactBtn": "Contact",
      "nav": [
        "Home",
        "Ingredients",
        "How to use",
        "Results",
        "Reviews",
        "Pricing"
      ],
      "hero": {
        "badgeTitle": "OUR PROMISE",
        "badgeSub": "SULFATE-FREE · PARABEN-FREE",
        "scrollHint": "SCROLL"
      },
      "footer": {
        "tagline": "Hair care that starts at the root — cleanse first, nourish after."
      },
      "ingredients": {
        "eyebrow": "INGREDIENTS",
        "t1": "What's",
        "t2": "Inside",
        "lead": "Two formulas that work together: one clears the way for the follicle, the other carries nutrients down to the root.",
        "items": [
          {
            "name": "Salicylic Acid 2%",
            "desc": "Dissolves the sebum and dead skin blocking the follicle opening"
          },
          {
            "name": "Tea tree oil",
            "desc": "Mildly antibacterial — calms itching and flaking on the scalp"
          },
          {
            "name": "Panthenol B5",
            "desc": "Soothes and rehydrates the scalp after a deep cleanse"
          },
          {
            "name": "Redensyl 3%",
            "desc": "Wakes dormant follicles and lengthens the growth phase"
          },
          {
            "name": "Caffeine & Biotin B7",
            "desc": "Improves circulation under the scalp and supplies the building blocks for new strands"
          },
          {
            "name": "Rosemary oil",
            "desc": "Helps reduce seasonal hair shedding"
          }
        ],
        "note": "Sample list — replace with the real ingredient panel from your product label."
      },
      "howto": {
        "eyebrow": "HOW TO USE",
        "t1": "Two",
        "t2": "Steps",
        "lead": "Nothing complicated. Cleanse a few times a week, nourish every night before bed.",
        "steps": [
          {
            "h": "Cleanse the roots",
            "p": "Wet the hair, take a coin-sized amount of gel, massage it straight onto the scalp for 60 seconds, then rinse thoroughly.",
            "em": "2–3 TIMES / WEEK"
          },
          {
            "h": "Apply serum to the scalp",
            "p": "With hair still damp, part it and trace the serum along the parting, especially where hair is thinning.",
            "em": "EVERY NIGHT"
          },
          {
            "h": "Massage for one minute",
            "p": "Work it in with your fingertips in small circles so it reaches the follicle. No need to rinse.",
            "em": "NO RINSING"
          }
        ]
      },
      "results": {
        "eyebrow": "RESULTS",
        "t1": "Week",
        "t2": "By Week",
        "lead": "Hair grows in cycles, not in days. Here is the timeline you should realistically expect.",
        "weeks": [
          {
            "b": "Week 2",
            "s": "Scalp clean, no more greasiness or itching"
          },
          {
            "b": "Week 4",
            "s": "Noticeably less hair falling out when you wash"
          },
          {
            "b": "Week 8",
            "s": "Baby hairs appearing in the thin areas"
          },
          {
            "b": "Week 12",
            "s": "Visibly denser hair"
          }
        ],
        "tiles": [
          {
            "lbl": "of users saw less shedding after 8 weeks"
          },
          {
            "lbl": "more new baby hairs than before use"
          },
          {
            "lbl": "weeks on average to see denser hair"
          }
        ],
        "note": "Illustrative figures — replace with your own survey results before publishing."
      },
      "reviews": {
        "eyebrow": "REVIEWS",
        "t1": "Real",
        "t2": "Voices",
        "items": [
          {
            "text": "My scalp was unbelievably oily — greasy the day after washing. Three weeks on step 1 and my hair felt light again, no more itching.",
            "meta": "3 months of use"
          },
          {
            "text": "I lost a lot of hair after giving birth and both temples thinned right out. Two months on the serum and there is a real layer of new growth coming through.",
            "meta": "2 months of use"
          },
          {
            "text": "The serum isn't greasy — leave it overnight and hair is still dry in the morning. Half a star off because the rosemary scent is strong for me.",
            "meta": "6 weeks of use"
          },
          {
            "text": "It only clicked once I used both. Before, I was only spraying serum onto a scalp that was still dirty, so it did almost nothing.",
            "meta": "4 months of use"
          }
        ],
        "note": "Sample reviews — replace with real customer feedback."
      },
      "pricing": {
        "eyebrow": "PRICING",
        "t1": "Start",
        "t2": "Today",
        "comboLabel": "Buy the pair",
        "comboBtn": "Buy the pair",
        "note": "Illustrative prices — replace with your real selling prices."
      },
      "lines": [
        {
          "name": "Root Recovery Set",
          "navLabel": "Recovery & Growth",
          "cta": "Buy the pair",
          "heroT1": "Awaken",
          "heroT2": "Roots"
        },
        {
          "name": "Root Care Shampoo",
          "navLabel": "Shampoo",
          "cta": "Buy shampoo",
          "heroT1": "Daily",
          "heroT2": "Clean",
          "ingredients": {
            "eyebrow": "INGREDIENTS",
            "t1": "Gentle",
            "t2": "Wash",
            "lead": "Coconut-derived foam instead of sulfates: clean enough for oily hair, without stripping the oil that protects your scalp.",
            "items": [
              {
                "name": "Coco Glucoside",
                "desc": "A coconut-based foaming agent. Rinses away sebum without leaving the scalp tight and dry"
              },
              {
                "name": "Panthenol B5 & Glycerin",
                "desc": "Holds water in the scalp and the hair shaft — softer after blow-drying, less dryness at the ends"
              },
              {
                "name": "Peppermint & tea tree",
                "desc": "Calms an itchy scalp, cool while you wash, helps keep oil-related flaking in check"
              }
            ]
          },
          "howto": {
            "eyebrow": "HOW TO USE",
            "t1": "Daily",
            "t2": "Ritual",
            "lead": "Washing well matters as much as what you wash with. Three things, under three minutes.",
            "steps": [
              {
                "h": "Wet the hair thoroughly",
                "p": "Rinse with warm water for about 30 seconds. Properly wet hair needs very little shampoo to lather.",
                "em": "WARM, NOT HOT"
              },
              {
                "h": "Massage the scalp for 60 seconds",
                "p": "Take a coin-sized amount, spread it between your palms, then massage with your fingertips — wash the scalp, not the lengths.",
                "em": "DAILY OR EVERY OTHER DAY"
              },
              {
                "h": "Rinse clean, pat dry",
                "p": "Rinse until the hair no longer feels slippery. Blot with a towel rather than rubbing — wet hair breaks easily.",
                "em": "BLOT, DON'T RUB"
              }
            ]
          },
          "results": {
            "eyebrow": "RESULTS",
            "t1": "Clean",
            "t2": "Scalp",
            "lead": "Shampoo doesn't grow hair. Its job is to keep the scalp in a state where hair can grow.",
            "weeks": [
              {
                "b": "First wash",
                "s": "Hair feels light, no greasy residue"
              },
              {
                "b": "Week 2",
                "s": "Less itching, less oil-related flaking"
              },
              {
                "b": "Week 4",
                "s": "Longer gap between washes"
              },
              {
                "b": "Week 8",
                "s": "Scalp settled, hair easier to style"
              }
            ],
            "tiles": [
              {
                "lbl": "sulfates and silicones in the formula"
              },
              {
                "lbl": "pH, close to the scalp's natural pH"
              },
              {
                "lbl": "lasts about 10 weeks washing every other day"
              }
            ],
            "note": "Illustrative figures — replace with your own lab results before publishing."
          },
          "reviews": {
            "eyebrow": "REVIEWS",
            "t1": "Real",
            "t2": "Voices",
            "items": [
              {
                "text": "My hair got greasy so fast I had to wash it daily. With this I can go every other day and my scalp stopped itching.",
                "meta": "2 months of use"
              },
              {
                "text": "My hair is colour-treated so I worried shampoo would fade it. Nearly three months in and the colour holds far better than with my old one.",
                "meta": "3 months of use"
              },
              {
                "text": "It lathers less than regular shampoo, which felt odd at first, but once I got used to it my hair is softer and lighter. Half a star off because the bottle is hard to squeeze near the end.",
                "meta": "6 weeks of use"
              }
            ],
            "note": "Sample reviews — replace with real customer feedback."
          },
          "pricing": {
            "eyebrow": "PRICING",
            "t1": "Start",
            "t2": "Today",
            "note": "Illustrative prices — replace with your real selling prices."
          }
        }
      ],
      "products": [
        {
          "cardSub": "Step 1 · Cleanse",
          "tag": "Step 1 · Cleanse the roots",
          "desc": "A deep cleansing gel for roots and follicles.<br>Lifts away the sebum, dirt and dead skin<br>clogging the pores on your scalp.",
          "priceSub": "Root & follicle cleansing gel · 200ml",
          "buyBtn": "Buy Deep Cleanse",
          "features": [
            "Lasts about 8 weeks",
            "Sulfate-free, paraben-free",
            "Suits oily and flaky scalps"
          ],
          "bottle": {
            "vn": "Root & follicle cleanser"
          }
        },
        {
          "cardSub": "Step 2 · Nourish",
          "tag": "Step 2 · Nourish & stimulate growth",
          "desc": "A serum that feeds the follicle from within.<br>It sinks into the root, encourages new growth<br>and reduces breakage day by day.",
          "priceSub": "Nourishing & growth-stimulating serum · 60ml",
          "buyBtn": "Buy Growth Serum",
          "features": [
            "Lasts about 6 weeks",
            "Non-greasy, no rinsing needed",
            "For both men and women"
          ],
          "bottle": {
            "vn": "Nourish & stimulate growth"
          }
        },
        {
          "cardSub": "Daily wash",
          "tag": "Gentle shampoo · sulfate-free",
          "desc": "A shampoo you can use every day.<br>Clean enough, while keeping the natural oil<br>that protects your scalp and roots.",
          "priceSub": "Gentle sulfate-free shampoo · 300ml",
          "buyBtn": "Buy shampoo",
          "features": [
            "Lasts about 10 weeks",
            "Sulfate-free, silicone-free",
            "Daily use, safe on coloured hair"
          ],
          "bottle": {
            "vn": "Gentle everyday shampoo"
          }
        }
      ],
      "ui": {
        "zalo": "Chat on Zalo",
        "goi": "Call",
        "lienHe": "Contact",
        "kenhMua": "Where to buy"
      },
      "contact": {
        "heading": "Order & advice",
        "note": "Message us on Zalo for free advice before you buy. Any field left blank is hidden automatically.",
        "hours": "8:00 – 21:00 · Monday – Sunday",
        "shopLabel": "Where to buy"
      }
    },
    "fr": {
      "contactBtn": "Contact",
      "nav": [
        "Accueil",
        "Composition",
        "Utilisation",
        "Résultats",
        "Avis",
        "Tarifs"
      ],
      "hero": {
        "badgeTitle": "NOTRE ENGAGEMENT",
        "badgeSub": "SANS SULFATE · SANS PARABÈNE",
        "scrollHint": "DÉFILER"
      },
      "footer": {
        "tagline": "Prendre soin des cheveux dès la racine — nettoyer d'abord, nourrir ensuite."
      },
      "ingredients": {
        "eyebrow": "COMPOSITION",
        "t1": "Au",
        "t2": "cœur",
        "lead": "Deux formules complémentaires : l'une dégage le passage vers le follicule, l'autre apporte les nutriments jusqu'à la racine.",
        "items": [
          {
            "name": "Acide salicylique 2 %",
            "desc": "Dissout le sébum et les cellules mortes qui obstruent l'ouverture du follicule"
          },
          {
            "name": "Huile d'arbre à thé",
            "desc": "Légèrement antibactérienne, apaise les démangeaisons et les pellicules"
          },
          {
            "name": "Panthénol B5",
            "desc": "Apaise et réhydrate le cuir chevelu après un nettoyage en profondeur"
          },
          {
            "name": "Redensyl 3 %",
            "desc": "Réveille les follicules endormis et prolonge la phase de croissance"
          },
          {
            "name": "Caféine & Biotine B7",
            "desc": "Stimule la circulation sous le cuir chevelu et fournit la matière première du cheveu"
          },
          {
            "name": "Huile de romarin",
            "desc": "Aide à limiter la chute saisonnière"
          }
        ],
        "note": "Liste d'exemple — remplacez-la par la composition réelle figurant sur votre étiquette."
      },
      "howto": {
        "eyebrow": "UTILISATION",
        "t1": "Deux",
        "t2": "gestes",
        "lead": "Rien de compliqué. Nettoyer quelques fois par semaine, nourrir chaque soir avant le coucher.",
        "steps": [
          {
            "h": "Nettoyer les racines",
            "p": "Mouillez les cheveux, prenez l'équivalent d'une pièce de gel, massez directement le cuir chevelu 60 secondes puis rincez soigneusement.",
            "em": "2–3 FOIS / SEMAINE"
          },
          {
            "h": "Appliquer le sérum",
            "p": "Sur cheveux encore humides, faites une raie et déposez le sérum le long de la raie, surtout sur les zones clairsemées.",
            "em": "CHAQUE SOIR"
          },
          {
            "h": "Masser une minute",
            "p": "Faites pénétrer du bout des doigts par petits cercles jusqu'au follicule. Pas besoin de rincer.",
            "em": "SANS RINÇAGE"
          }
        ]
      },
      "results": {
        "eyebrow": "RÉSULTATS",
        "t1": "Semaine",
        "t2": "après semaine",
        "lead": "Le cheveu pousse par cycles, pas au jour le jour. Voici le calendrier auquel vous attendre.",
        "weeks": [
          {
            "b": "Semaine 2",
            "s": "Cuir chevelu net, plus de sensation grasse ni de démangeaisons"
          },
          {
            "b": "Semaine 4",
            "s": "Nettement moins de cheveux tombés au lavage"
          },
          {
            "b": "Semaine 8",
            "s": "Duvet visible sur les zones clairsemées"
          },
          {
            "b": "Semaine 12",
            "s": "Chevelure visiblement plus dense"
          }
        ],
        "tiles": [
          {
            "lbl": "des utilisateurs constatent moins de chute après 8 semaines"
          },
          {
            "lbl": "plus de nouveaux cheveux qu'avant utilisation"
          },
          {
            "lbl": "semaines en moyenne pour voir la densité augmenter"
          }
        ],
        "note": "Chiffres indicatifs — remplacez-les par vos propres résultats d'étude avant publication."
      },
      "reviews": {
        "eyebrow": "AVIS",
        "t1": "Vraies",
        "t2": "voix",
        "items": [
          {
            "text": "J'avais le cuir chevelu extrêmement gras, regraissé dès le lendemain du shampooing. Trois semaines avec l'étape 1 et mes cheveux sont légers, plus aucune démangeaison.",
            "meta": "3 mois d'utilisation"
          },
          {
            "text": "J'ai beaucoup perdu après mon accouchement, les deux tempes s'étaient dégarnies. Après deux mois de sérum, il y a vraiment un duvet qui repousse.",
            "meta": "2 mois d'utilisation"
          },
          {
            "text": "Le sérum ne graisse pas : posé le soir, les cheveux sont secs au réveil. Une demi-étoile en moins car l'odeur de romarin est forte pour moi.",
            "meta": "6 semaines d'utilisation"
          },
          {
            "text": "La différence est venue en utilisant les deux. Avant, je mettais le sérum sur un cuir chevelu encore sale, autant dire pour rien.",
            "meta": "4 mois d'utilisation"
          }
        ],
        "note": "Avis d'exemple — remplacez-les par de vrais retours clients."
      },
      "pricing": {
        "eyebrow": "TARIFS",
        "t1": "Commencer",
        "t2": "aujourd'hui",
        "comboLabel": "Le duo complet",
        "comboBtn": "Acheter le duo",
        "note": "Prix indicatifs — remplacez-les par vos prix de vente réels."
      },
      "lines": [
        {
          "name": "Coffret Racines",
          "navLabel": "Réparation & Pousse",
          "cta": "Acheter le duo",
          "heroT1": "Racines",
          "heroT2": "vivantes"
        },
        {
          "name": "Shampooing Racines",
          "navLabel": "Shampooing",
          "cta": "Acheter le shampooing",
          "heroT1": "Lavage",
          "heroT2": "doux",
          "ingredients": {
            "eyebrow": "COMPOSITION",
            "t1": "Mousse",
            "t2": "douce",
            "lead": "Une mousse issue de la noix de coco plutôt que des sulfates : assez nettoyante pour les cheveux gras, sans retirer le film protecteur du cuir chevelu.",
            "items": [
              {
                "name": "Coco Glucoside",
                "desc": "Agent moussant dérivé de la noix de coco. Élimine le sébum sans laisser le cuir chevelu tiraillé"
              },
              {
                "name": "Panthénol B5 & Glycérine",
                "desc": "Retient l'eau dans le cuir chevelu et la fibre : cheveux souples après séchage, pointes moins sèches"
              },
              {
                "name": "Menthe poivrée & arbre à thé",
                "desc": "Apaise les démangeaisons, effet frais pendant le lavage, aide à limiter les pellicules grasses"
              }
            ]
          },
          "howto": {
            "eyebrow": "UTILISATION",
            "t1": "Rituel",
            "t2": "quotidien",
            "lead": "Bien se laver compte autant que le produit utilisé. Trois gestes, moins de trois minutes.",
            "steps": [
              {
                "h": "Mouiller abondamment",
                "p": "Rincez à l'eau tiède une trentaine de secondes. Sur cheveux bien mouillés, très peu de shampooing suffit à mousser.",
                "em": "TIÈDE, PAS CHAUD"
              },
              {
                "h": "Masser 60 secondes",
                "p": "Prenez l'équivalent d'une pièce, émulsionnez entre les paumes puis massez du bout des doigts — lavez le cuir chevelu, pas les longueurs.",
                "em": "TOUS LES JOURS OU UN JOUR SUR DEUX"
              },
              {
                "h": "Rincer, éponger",
                "p": "Rincez jusqu'à ce que les cheveux ne soient plus glissants. Épongez avec une serviette au lieu de frotter : le cheveu mouillé casse facilement.",
                "em": "ÉPONGER, NE PAS FROTTER"
              }
            ]
          },
          "results": {
            "eyebrow": "RÉSULTATS",
            "t1": "Cuir",
            "t2": "chevelu sain",
            "lead": "Un shampooing ne fait pas pousser les cheveux. Son rôle est de maintenir un cuir chevelu où ils peuvent pousser.",
            "weeks": [
              {
                "b": "Dès le 1er lavage",
                "s": "Cheveux légers, plus de sensation grasse"
              },
              {
                "b": "Semaine 2",
                "s": "Moins de démangeaisons et de pellicules grasses"
              },
              {
                "b": "Semaine 4",
                "s": "Plus de temps entre deux shampooings"
              },
              {
                "b": "Semaine 8",
                "s": "Cuir chevelu stabilisé, coiffage plus facile"
              }
            ],
            "tiles": [
              {
                "lbl": "de sulfates et de silicones dans la formule"
              },
              {
                "lbl": "de pH, proche du pH naturel du cuir chevelu"
              },
              {
                "lbl": "environ 10 semaines à raison d'un lavage sur deux jours"
              }
            ],
            "note": "Chiffres indicatifs — remplacez-les par vos propres analyses avant publication."
          },
          "reviews": {
            "eyebrow": "AVIS",
            "t1": "Vraies",
            "t2": "voix",
            "items": [
              {
                "text": "Mes cheveux regraissaient si vite que je lavais tous les jours. Avec celui-ci je tiens un jour sur deux et mon cuir chevelu ne gratte plus.",
                "meta": "2 mois d'utilisation"
              },
              {
                "text": "Mes cheveux sont colorés et je craignais que le shampooing les délave. Presque trois mois plus tard, la couleur tient bien mieux qu'avec mon ancien.",
                "meta": "3 mois d'utilisation"
              },
              {
                "text": "Il mousse moins qu'un shampooing classique, déroutant au début, mais les cheveux sont plus souples et plus légers. Une demi-étoile en moins car le flacon est difficile à presser vers la fin.",
                "meta": "6 semaines d'utilisation"
              }
            ],
            "note": "Avis d'exemple — remplacez-les par de vrais retours clients."
          },
          "pricing": {
            "eyebrow": "TARIFS",
            "t1": "Commencer",
            "t2": "aujourd'hui",
            "note": "Prix indicatifs — remplacez-les par vos prix de vente réels."
          }
        }
      ],
      "products": [
        {
          "cardSub": "Étape 1 · Nettoyer",
          "tag": "Étape 1 · Nettoyer les racines",
          "desc": "Gel nettoyant pour racines et follicules.<br>Élimine sébum, impuretés et cellules mortes<br>qui obstruent les pores du cuir chevelu.",
          "priceSub": "Gel nettoyant racines & follicules · 200 ml",
          "buyBtn": "Acheter Deep Cleanse",
          "features": [
            "Environ 8 semaines d'utilisation",
            "Sans sulfate, sans parabène",
            "Pour cuir chevelu gras ou à pellicules"
          ],
          "bottle": {
            "vn": "Nettoyant racines & follicules"
          }
        },
        {
          "cardSub": "Étape 2 · Nourrir",
          "tag": "Étape 2 · Nourrir & stimuler la pousse",
          "desc": "Un sérum qui nourrit le follicule.<br>Il pénètre jusqu'à la racine, relance la pousse<br>et réduit la casse jour après jour.",
          "priceSub": "Sérum nourrissant & stimulant la pousse · 60 ml",
          "buyBtn": "Acheter Growth Serum",
          "features": [
            "Environ 6 semaines d'utilisation",
            "Non gras, sans rinçage",
            "Pour les femmes comme pour les hommes"
          ],
          "bottle": {
            "vn": "Nourrit & stimule la pousse"
          }
        },
        {
          "cardSub": "Lavage quotidien",
          "tag": "Shampooing doux · sans sulfate",
          "desc": "Un shampooing pour tous les jours.<br>Assez nettoyant, il garde le film naturel<br>qui protège le cuir chevelu et les racines.",
          "priceSub": "Shampooing doux sans sulfate · 300 ml",
          "buyBtn": "Acheter le shampooing",
          "features": [
            "Environ 10 semaines d'utilisation",
            "Sans sulfate, sans silicone",
            "Usage quotidien, convient aux cheveux colorés"
          ],
          "bottle": {
            "vn": "Shampooing doux quotidien"
          }
        }
      ],
      "ui": {
        "zalo": "Écrire sur Zalo",
        "goi": "Appeler",
        "lienHe": "Contact",
        "kenhMua": "Où acheter"
      },
      "contact": {
        "heading": "Commande & conseils",
        "note": "Écrivez-nous sur Zalo pour un conseil gratuit avant d'acheter. Tout champ laissé vide est masqué automatiquement.",
        "hours": "8h00 – 21h00 · Du lundi au dimanche",
        "shopLabel": "Où acheter"
      }
    },
    "it": {
      "contactBtn": "Contatti",
      "nav": [
        "Home",
        "Ingredienti",
        "Come si usa",
        "Risultati",
        "Recensioni",
        "Prezzi"
      ],
      "hero": {
        "badgeTitle": "IL NOSTRO IMPEGNO",
        "badgeSub": "SENZA SOLFATI · SENZA PARABENI",
        "scrollHint": "SCORRI"
      },
      "footer": {
        "tagline": "Cura dei capelli che parte dalla radice — prima si pulisce, poi si nutre."
      },
      "ingredients": {
        "eyebrow": "INGREDIENTI",
        "t1": "Nel",
        "t2": "cuore",
        "lead": "Due formule che si completano: una libera la strada al follicolo, l'altra porta i nutrienti fino alla radice.",
        "items": [
          {
            "name": "Acido salicilico 2%",
            "desc": "Scioglie sebo e cellule morte che ostruiscono l'apertura del follicolo"
          },
          {
            "name": "Olio di tea tree",
            "desc": "Leggermente antibatterico, calma prurito e forfora del cuoio capelluto"
          },
          {
            "name": "Pantenolo B5",
            "desc": "Lenisce e reidrata il cuoio capelluto dopo la pulizia profonda"
          },
          {
            "name": "Redensyl 3%",
            "desc": "Risveglia i follicoli dormienti e allunga la fase di crescita"
          },
          {
            "name": "Caffeina & Biotina B7",
            "desc": "Migliora la circolazione sotto il cuoio capelluto e fornisce il materiale per il nuovo capello"
          },
          {
            "name": "Olio di rosmarino",
            "desc": "Aiuta a ridurre la caduta stagionale"
          }
        ],
        "note": "Elenco di esempio — sostituiscilo con la lista ingredienti reale della tua etichetta."
      },
      "howto": {
        "eyebrow": "COME SI USA",
        "t1": "Due",
        "t2": "passaggi",
        "lead": "Niente di complicato. Pulire qualche volta a settimana, nutrire ogni sera prima di dormire.",
        "steps": [
          {
            "h": "Pulire le radici",
            "p": "Bagna i capelli, preleva una quantità di gel grande come una moneta, massaggia direttamente il cuoio capelluto per 60 secondi e risciacqua a fondo.",
            "em": "2–3 VOLTE / SETTIMANA"
          },
          {
            "h": "Applicare il siero",
            "p": "Con i capelli ancora umidi, fai una riga e distribuisci il siero lungo la riga, soprattutto dove i capelli sono diradati.",
            "em": "OGNI SERA"
          },
          {
            "h": "Massaggiare un minuto",
            "p": "Fai penetrare con i polpastrelli, con piccoli movimenti circolari, fino al follicolo. Non serve risciacquare.",
            "em": "SENZA RISCIACQUO"
          }
        ]
      },
      "results": {
        "eyebrow": "RISULTATI",
        "t1": "Settimana",
        "t2": "dopo settimana",
        "lead": "I capelli crescono per cicli, non giorno per giorno. Ecco i tempi realistici da aspettarsi.",
        "weeks": [
          {
            "b": "Settimana 2",
            "s": "Cuoio capelluto pulito, niente più unto né prurito"
          },
          {
            "b": "Settimana 4",
            "s": "Molti meno capelli persi durante il lavaggio"
          },
          {
            "b": "Settimana 8",
            "s": "Capelli nuovi visibili nelle zone diradate"
          },
          {
            "b": "Settimana 12",
            "s": "Chioma visibilmente più folta"
          }
        ],
        "tiles": [
          {
            "lbl": "degli utenti nota meno caduta dopo 8 settimane"
          },
          {
            "lbl": "capelli nuovi in più rispetto a prima dell'uso"
          },
          {
            "lbl": "settimane in media per vedere più densità"
          }
        ],
        "note": "Dati indicativi — sostituiscili con i risultati reali della tua indagine prima di pubblicare."
      },
      "reviews": {
        "eyebrow": "RECENSIONI",
        "t1": "Voci",
        "t2": "vere",
        "items": [
          {
            "text": "Avevo il cuoio capelluto grassissimo, già unto il giorno dopo lo shampoo. Tre settimane con il passaggio 1 e i capelli sono leggeri, prurito sparito.",
            "meta": "3 mesi di utilizzo"
          },
          {
            "text": "Ho perso molti capelli dopo il parto, con le tempie molto diradate. Dopo due mesi di siero c'è davvero una peluria nuova che ricresce.",
            "meta": "2 mesi di utilizzo"
          },
          {
            "text": "Il siero non unge: messo la sera, la mattina i capelli sono asciutti. Mezza stella in meno perché il profumo di rosmarino per me è forte.",
            "meta": "6 settimane di utilizzo"
          },
          {
            "text": "La differenza si è vista solo usandoli entrambi. Prima mettevo il siero su un cuoio capelluto ancora sporco, quindi non serviva quasi a nulla.",
            "meta": "4 mesi di utilizzo"
          }
        ],
        "note": "Recensioni di esempio — sostituiscile con feedback reali dei clienti."
      },
      "pricing": {
        "eyebrow": "PREZZI",
        "t1": "Inizia",
        "t2": "oggi",
        "comboLabel": "Il duo completo",
        "comboBtn": "Acquista il duo",
        "note": "Prezzi indicativi — sostituiscili con i tuoi prezzi di vendita reali."
      },
      "lines": [
        {
          "name": "Kit Radici",
          "navLabel": "Riparazione & Crescita",
          "cta": "Acquista il duo",
          "heroT1": "Radici",
          "heroT2": "vive"
        },
        {
          "name": "Shampoo Radici",
          "navLabel": "Shampoo",
          "cta": "Acquista lo shampoo",
          "heroT1": "Lavaggio",
          "heroT2": "delicato",
          "ingredients": {
            "eyebrow": "INGREDIENTI",
            "t1": "Schiuma",
            "t2": "delicata",
            "lead": "Schiuma derivata dal cocco al posto dei solfati: pulisce abbastanza per i capelli grassi, senza togliere il film che protegge il cuoio capelluto.",
            "items": [
              {
                "name": "Coco Glucoside",
                "desc": "Tensioattivo di origine cocco. Rimuove il sebo senza lasciare il cuoio capelluto secco e tirato"
              },
              {
                "name": "Pantenolo B5 & Glicerina",
                "desc": "Trattiene acqua nel cuoio capelluto e nella fibra: capelli morbidi dopo l'asciugatura, punte meno secche"
              },
              {
                "name": "Menta piperita & tea tree",
                "desc": "Calma il prurito, effetto fresco durante il lavaggio, aiuta a controllare la forfora grassa"
              }
            ]
          },
          "howto": {
            "eyebrow": "COME SI USA",
            "t1": "Rituale",
            "t2": "quotidiano",
            "lead": "Lavare bene conta quanto il prodotto che usi. Tre gesti, meno di tre minuti.",
            "steps": [
              {
                "h": "Bagnare bene i capelli",
                "p": "Risciacqua con acqua tiepida per circa 30 secondi. Su capelli ben bagnati serve pochissimo shampoo per fare schiuma.",
                "em": "TIEPIDA, NON CALDA"
              },
              {
                "h": "Massaggiare 60 secondi",
                "p": "Preleva una quantità grande come una moneta, emulsiona tra i palmi e massaggia con i polpastrelli — lava il cuoio capelluto, non le lunghezze.",
                "em": "OGNI GIORNO O A GIORNI ALTERNI"
              },
              {
                "h": "Risciacquare e tamponare",
                "p": "Risciacqua finché i capelli non scivolano più. Tampona con l'asciugamano invece di strofinare: bagnati si spezzano facilmente.",
                "em": "TAMPONA, NON STROFINARE"
              }
            ]
          },
          "results": {
            "eyebrow": "RISULTATI",
            "t1": "Cuoio",
            "t2": "capelluto sano",
            "lead": "Lo shampoo non fa crescere i capelli. Il suo compito è mantenere il cuoio capelluto nelle condizioni in cui possono crescere.",
            "weeks": [
              {
                "b": "Dal 1° lavaggio",
                "s": "Capelli leggeri, niente sensazione di unto"
              },
              {
                "b": "Settimana 2",
                "s": "Meno prurito e meno forfora grassa"
              },
              {
                "b": "Settimana 4",
                "s": "Più tempo tra un lavaggio e l'altro"
              },
              {
                "b": "Settimana 8",
                "s": "Cuoio capelluto stabile, capelli più facili da gestire"
              }
            ],
            "tiles": [
              {
                "lbl": "di solfati e siliconi nella formula"
              },
              {
                "lbl": "di pH, vicino al pH naturale del cuoio capelluto"
              },
              {
                "lbl": "circa 10 settimane lavando a giorni alterni"
              }
            ],
            "note": "Dati indicativi — sostituiscili con le tue analisi reali prima di pubblicare."
          },
          "reviews": {
            "eyebrow": "RECENSIONI",
            "t1": "Voci",
            "t2": "vere",
            "items": [
              {
                "text": "I capelli mi si ungevano così in fretta che li lavavo ogni giorno. Con questo reggo a giorni alterni e il cuoio capelluto non prude più.",
                "meta": "2 mesi di utilizzo"
              },
              {
                "text": "Ho i capelli colorati e temevo che lo shampoo li slavasse. Quasi tre mesi dopo, il colore tiene molto meglio del precedente.",
                "meta": "3 mesi di utilizzo"
              },
              {
                "text": "Fa meno schiuma di uno shampoo normale, all'inizio strano, ma poi i capelli sono più morbidi e leggeri. Mezza stella in meno perché il flacone è scomodo da spremere verso la fine.",
                "meta": "6 settimane di utilizzo"
              }
            ],
            "note": "Recensioni di esempio — sostituiscile con feedback reali dei clienti."
          },
          "pricing": {
            "eyebrow": "PREZZI",
            "t1": "Inizia",
            "t2": "oggi",
            "note": "Prezzi indicativi — sostituiscili con i tuoi prezzi di vendita reali."
          }
        }
      ],
      "products": [
        {
          "cardSub": "Passo 1 · Pulizia",
          "tag": "Passo 1 · Pulizia delle radici",
          "desc": "Gel detergente profondo per radici e follicoli.<br>Rimuove sebo, impurità e cellule morte<br>che ostruiscono i pori del cuoio capelluto.",
          "priceSub": "Gel detergente radici & follicoli · 200 ml",
          "buyBtn": "Acquista Deep Cleanse",
          "features": [
            "Dura circa 8 settimane",
            "Senza solfati, senza parabeni",
            "Per cuoio capelluto grasso o con forfora"
          ],
          "bottle": {
            "vn": "Detergente radici & follicoli"
          }
        },
        {
          "cardSub": "Passo 2 · Nutrimento",
          "tag": "Passo 2 · Nutre & stimola la crescita",
          "desc": "Un siero che nutre il follicolo dall'interno.<br>Penetra fino alla radice, stimola la ricrescita<br>e riduce la rottura giorno dopo giorno.",
          "priceSub": "Siero nutriente e stimolante · 60 ml",
          "buyBtn": "Acquista Growth Serum",
          "features": [
            "Dura circa 6 settimane",
            "Non unge, non serve risciacquare",
            "Per donne e uomini"
          ],
          "bottle": {
            "vn": "Nutre e stimola la crescita"
          }
        },
        {
          "cardSub": "Lavaggio quotidiano",
          "tag": "Shampoo delicato · senza solfati",
          "desc": "Uno shampoo che puoi usare ogni giorno.<br>Pulisce il giusto, mantenendo il film naturale<br>che protegge cuoio capelluto e radici.",
          "priceSub": "Shampoo delicato senza solfati · 300 ml",
          "buyBtn": "Acquista lo shampoo",
          "features": [
            "Dura circa 10 settimane",
            "Senza solfati, senza siliconi",
            "Uso quotidiano, adatto ai capelli colorati"
          ],
          "bottle": {
            "vn": "Shampoo delicato quotidiano"
          }
        }
      ],
      "ui": {
        "zalo": "Scrivi su Zalo",
        "goi": "Chiama",
        "lienHe": "Contatti",
        "kenhMua": "Dove acquistare"
      },
      "contact": {
        "heading": "Ordini & consulenza",
        "note": "Scrivici su Zalo per una consulenza gratuita prima di acquistare. I campi lasciati vuoti vengono nascosti automaticamente.",
        "hours": "8:00 – 21:00 · Da lunedì a domenica",
        "shopLabel": "Dove acquistare"
      }
    },
    "ja": {
      "contactBtn": "お問い合わせ",
      "nav": [
        "ホーム",
        "成分",
        "使い方",
        "効果",
        "レビュー",
        "価格"
      ],
      "hero": {
        "badgeTitle": "お約束",
        "badgeSub": "サルフェート不使用・パラベン不使用",
        "scrollHint": "スクロール"
      },
      "footer": {
        "tagline": "髪は根元から。まず洗い、そして育てる。"
      },
      "ingredients": {
        "eyebrow": "成分",
        "t1": "Inside",
        "t2": "the bottle",
        "lead": "補い合う二つの処方。片方は毛穴の詰まりを取り除き、もう片方は栄養を根元まで届けます。",
        "items": [
          {
            "name": "サリチル酸 2%",
            "desc": "毛穴をふさぐ皮脂と古い角質を溶かします"
          },
          {
            "name": "ティーツリーオイル",
            "desc": "おだやかな抗菌作用で、頭皮のかゆみとフケをやわらげます"
          },
          {
            "name": "パンテノール B5",
            "desc": "ディープクレンジング後の頭皮をしずめ、うるおいを戻します"
          },
          {
            "name": "レデンシル 3%",
            "desc": "眠っている毛包を目覚めさせ、成長期を長く保ちます"
          },
          {
            "name": "カフェイン & ビオチン B7",
            "desc": "頭皮の血行を促し、新しい髪をつくる材料を届けます"
          },
          {
            "name": "ローズマリーオイル",
            "desc": "季節性の抜け毛をやわらげます"
          }
        ],
        "note": "サンプル表示です。実際の製品ラベルの全成分に差し替えてください。"
      },
      "howto": {
        "eyebrow": "使い方",
        "t1": "Two",
        "t2": "Steps",
        "lead": "むずかしくありません。週に数回洗い、毎晩ねる前に育てる。それだけです。",
        "steps": [
          {
            "h": "根元を洗う",
            "p": "髪をぬらし、10円玉ほどのジェルを取り、頭皮に直接60秒なじませてから、しっかりすすぎます。",
            "em": "週 2〜3 回"
          },
          {
            "h": "頭皮に美容液をつける",
            "p": "髪が少し湿った状態で分け目をつくり、分け目に沿って、とくに髪の薄い部分に美容液をなじませます。",
            "em": "毎晩"
          },
          {
            "h": "1分マッサージ",
            "p": "指の腹で小さく円を描くようにして毛包まで届けます。洗い流す必要はありません。",
            "em": "洗い流し不要"
          }
        ]
      },
      "results": {
        "eyebrow": "効果",
        "t1": "Week",
        "t2": "By Week",
        "lead": "髪は日ごとではなく周期で伸びます。現実的な目安をお伝えします。",
        "weeks": [
          {
            "b": "2週目",
            "s": "頭皮がすっきりし、ベタつきとかゆみが落ち着く"
          },
          {
            "b": "4週目",
            "s": "洗うときの抜け毛が目に見えて減る"
          },
          {
            "b": "8週目",
            "s": "薄かった部分に産毛が生えてくる"
          },
          {
            "b": "12週目",
            "s": "見てわかるほど髪の密度が上がる"
          }
        ],
        "tiles": [
          {
            "lbl": "8週間後に抜け毛の減少を実感した人の割合"
          },
          {
            "lbl": "使用前とくらべた新しい産毛の本数"
          },
          {
            "lbl": "密度の変化を感じるまでの平均週数"
          }
        ],
        "note": "参考値です。公開前にご自身の調査結果に差し替えてください。"
      },
      "reviews": {
        "eyebrow": "レビュー",
        "t1": "Real",
        "t2": "Voices",
        "items": [
          {
            "text": "頭皮がとても脂っぽく、洗った翌日にはもうベタついていました。ステップ1を三週間つづけたら髪が軽くなり、かゆみもなくなりました。",
            "meta": "3か月使用"
          },
          {
            "text": "出産後に髪がかなり抜け、こめかみの両側が薄くなりました。美容液を二か月つづけたら、細いけれど確かに新しい毛が生えてきています。",
            "meta": "2か月使用"
          },
          {
            "text": "美容液はベタつきません。夜つけても朝には乾いています。ローズマリーの香りが私には少し強いので0.5減点。",
            "meta": "6週間使用"
          },
          {
            "text": "二つそろえて初めて違いが出ました。以前は汚れた頭皮に美容液をつけていたので、ほとんど意味がなかったのだと思います。",
            "meta": "4か月使用"
          }
        ],
        "note": "サンプルのレビューです。実際のお客様の声に差し替えてください。"
      },
      "pricing": {
        "eyebrow": "価格",
        "t1": "Start",
        "t2": "Today",
        "comboLabel": "2本セットで購入",
        "comboBtn": "セットを購入",
        "note": "参考価格です。実際の販売価格に差し替えてください。"
      },
      "lines": [
        {
          "name": "根元ケアセット",
          "navLabel": "補修 & 育毛",
          "cta": "セットを購入",
          "heroT1": "Awaken",
          "heroT2": "Roots"
        },
        {
          "name": "根元ケアシャンプー",
          "navLabel": "シャンプー",
          "cta": "シャンプーを購入",
          "heroT1": "Daily",
          "heroT2": "Clean",
          "ingredients": {
            "eyebrow": "成分",
            "t1": "Gentle",
            "t2": "Wash",
            "lead": "サルフェートの代わりにココナッツ由来の泡を。脂っぽい髪にも十分な洗浄力で、頭皮を守る皮脂は残します。",
            "items": [
              {
                "name": "ココグルコシド",
                "desc": "ココナッツ由来の洗浄成分。皮脂を落としながら、頭皮のつっぱりを残しません"
              },
              {
                "name": "パンテノール B5 & グリセリン",
                "desc": "頭皮と髪に水分を保ち、乾かしたあともやわらかく、毛先のパサつきをおさえます"
              },
              {
                "name": "ペパーミント & ティーツリー",
                "desc": "かゆい頭皮をしずめ、洗っている間はひんやり。皮脂によるフケをおさえます"
              }
            ]
          },
          "howto": {
            "eyebrow": "使い方",
            "t1": "Daily",
            "t2": "Ritual",
            "lead": "何で洗うかと同じくらい、どう洗うかが大切です。三つの手順、三分もかかりません。",
            "steps": [
              {
                "h": "しっかりぬらす",
                "p": "ぬるま湯で30秒ほどすすぎます。よくぬれていれば、少量のシャンプーで十分に泡立ちます。",
                "em": "ぬるま湯で、熱すぎない"
              },
              {
                "h": "頭皮を60秒マッサージ",
                "p": "10円玉ほどを手のひらでのばし、指の腹でマッサージします。洗うのは頭皮で、毛先ではありません。",
                "em": "毎日、または一日おき"
              },
              {
                "h": "すすいで、押さえて乾かす",
                "p": "ぬるつきがなくなるまですすぎます。こすらずタオルで押さえて。ぬれた髪はとても切れやすいからです。",
                "em": "こすらず、押さえる"
              }
            ]
          },
          "results": {
            "eyebrow": "効果",
            "t1": "Clean",
            "t2": "Scalp",
            "lead": "シャンプーが髪を生やすわけではありません。髪が育つ状態に頭皮を保つことが役目です。",
            "weeks": [
              {
                "b": "1回目から",
                "s": "髪が軽く、ベタつきが残らない"
              },
              {
                "b": "2週目",
                "s": "かゆみと皮脂によるフケが落ち着く"
              },
              {
                "b": "4週目",
                "s": "洗う間隔をあけられるようになる"
              },
              {
                "b": "8週目",
                "s": "頭皮が安定し、髪がまとまりやすくなる"
              }
            ],
            "tiles": [
              {
                "lbl": "処方に含まれるサルフェートとシリコーンの量"
              },
              {
                "lbl": "頭皮本来の pH に近い数値"
              },
              {
                "lbl": "一日おきに洗って約10週間分"
              }
            ],
            "note": "参考値です。公開前にご自身の試験結果に差し替えてください。"
          },
          "reviews": {
            "eyebrow": "レビュー",
            "t1": "Real",
            "t2": "Voices",
            "items": [
              {
                "text": "すぐ脂っぽくなるので毎日洗っていました。これに変えてから一日おきでも平気になり、頭皮のかゆみもなくなりました。",
                "meta": "2か月使用"
              },
              {
                "text": "カラーをしているので色落ちが心配でしたが、三か月近く使って以前のものよりずっと色もちが良いです。",
                "meta": "3か月使用"
              },
              {
                "text": "普通のシャンプーより泡立ちは控えめで最初は戸惑いましたが、慣れると髪がやわらかく軽くなりました。最後のほうはボトルが押しにくいので0.5減点。",
                "meta": "6週間使用"
              }
            ],
            "note": "サンプルのレビューです。実際のお客様の声に差し替えてください。"
          },
          "pricing": {
            "eyebrow": "価格",
            "t1": "Start",
            "t2": "Today",
            "note": "参考価格です。実際の販売価格に差し替えてください。"
          }
        }
      ],
      "products": [
        {
          "cardSub": "ステップ 1 · 洗う",
          "tag": "ステップ 1 · 根元を洗う",
          "desc": "根元と毛包のためのディープクレンジングジェル。<br>頭皮の毛穴をふさぐ皮脂・汚れ・古い角質を<br>やさしく取り除きます。",
          "priceSub": "根元・毛包用クレンジングジェル · 200ml",
          "buyBtn": "Deep Cleanse を購入",
          "features": [
            "約8週間分",
            "サルフェート・パラベン不使用",
            "脂性・フケが気になる頭皮に"
          ],
          "bottle": {
            "vn": "根元と毛包を洗う"
          }
        },
        {
          "cardSub": "ステップ 2 · 育てる",
          "tag": "ステップ 2 · 育毛と栄養",
          "desc": "毛包を内側から育てる美容液。<br>根元まで浸透して新しい髪の成長を促し、<br>切れ毛を日ごとに減らします。",
          "priceSub": "育毛美容液 · 60ml",
          "buyBtn": "Growth Serum を購入",
          "features": [
            "約6週間分",
            "ベタつかず、洗い流し不要",
            "男女どちらにも"
          ],
          "bottle": {
            "vn": "髪を育てる美容液"
          }
        },
        {
          "cardSub": "毎日のシャンプー",
          "tag": "やさしいシャンプー · サルフェート不使用",
          "desc": "毎日使えるシャンプーです。<br>必要なだけ洗い、頭皮と根元を守る<br>自然な皮脂は残します。",
          "priceSub": "サルフェート不使用のやさしいシャンプー · 300ml",
          "buyBtn": "シャンプーを購入",
          "features": [
            "約10週間分",
            "サルフェート・シリコーン不使用",
            "毎日使えて、カラー毛にも"
          ],
          "bottle": {
            "vn": "毎日使えるやさしいシャンプー"
          }
        }
      ],
      "ui": {
        "zalo": "Zaloで連絡",
        "goi": "電話",
        "lienHe": "お問い合わせ",
        "kenhMua": "購入できる場所"
      },
      "contact": {
        "heading": "ご注文・ご相談",
        "note": "ご購入前の無料相談はZaloからどうぞ。空欄の項目は自動的に非表示になります。",
        "hours": "8:00 – 21:00 · 月曜〜日曜",
        "shopLabel": "購入できる場所"
      }
    }
  },
  "ui": {
    "zalo": "Nhắn Zalo",
    "goi": "Gọi",
    "lienHe": "Liên hệ",
    "kenhMua": "Kênh mua hàng"
  },
  "contact": {}
};
